...
# ==== 在这里插入 ====
def safe_enum_lookup(type_str: str,
                     default_obj=ObjectType.FILE_OBJECT_BLOCK,
                     default_subj=ObjectType.SUBJECT_PROCESS):
    """
    把日志里的类型字符串安全映射到 ObjectType 的 .value；
    若不存在则根据前缀粗略归类，不抛 KeyError。
    """
    members = ObjectType.__members__
    if type_str in members:
        return members[type_str].value

    # 简单前缀归类，可按实际需要再细分
    if type_str.startswith("FILE_OBJECT"):
        return default_obj.value
    if type_str.endswith("Process") or "SUBJECT" in type_str:
        return default_subj.value
    if "NET" in type_str or "Socket" in type_str:
        return ObjectType.NETFLOW_OBJECT.value

    # 兜底：全当 FILE_OBJECT_BLOCK
    return default_obj.value
# ====================
...
class DARPAHandler(BaseProcessor):
    def load(self):
        ...
        for scene, category_data in json_map.items():
            ...
            for category, json_files in category_data.items():
                ...
                # scene_category = f"\\{scene}\\{category}"
    ...
    def build_graph(self):
        ...
        for _, row in self.use_df.iterrows():
            ...
            # =======修改后：======
            actor_idx = get_or_add_node(G, actor_id, safe_enum_lookup(row['actor_type'], default_subj=ObjectType.SUBJECT_PROCESS), properties)
            object_idx = get_or_add_node(G, object_id, safe_enum_lookup(row['object'], default_obj=ObjectType.FILE_OBJECT_BLOCK), properties)
            # ===================
