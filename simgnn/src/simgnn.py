...
class SimGNN(torch.nn.Module):
    ...
    def __init__(self, args, number_of_labels):
        ...
        self.number_labels = number_of_labels   #输入的特征的个数（维度）
    ...
    def setup_layers(self):
        ...
        # 即三个GCN层，GCN是pyg自带的（PyTorch Geometric）
        self.convolution_1 = GCNConv(self.number_labels, self.args.filters_1)   #输入的是节点特征的维度，第一个GCN的输出维度是filters_1
        self.convolution_2 = GCNConv(self.args.filters_1, self.args.filters_2)  #第二个GCN的输入的维度是filters_1，输出的维度是filters_2
        ...
        self.tensor_network = TenorNetworkModule(self.args)     #《张量神经网络》
        #再经过两个全连接层（灰色图的那坨）
    ...
    def convolutional_pass(self, edge_index, features):
        ...
        #第一层GCN
        features = self.convolution_1(features, edge_index) #传入features和edge_index两个矩阵  得到一个特征值features
        features = torch.nn.functional.relu(features)   #激活函数层ReLU
    ...
    def forward(self, data):    #输入的是处理好的数据格式 即 Mytest.txt最后
        ...
        abstract_features_1 = self.convolutional_pass(edge_index_1, features_1) #把图1丢尽GCN，进行卷积 得到矩阵即图片的Node-embedding
...
class SimGNNTrainer(object):
    ...
    """
    def __init__(self, args):#查看如何定义的       args刚才输入的参数
        """
        :param args: Arguments object.
        """
    ...
    def setup_model(self):
        ...
        self.model = SimGNN(self.args, self.number_of_labels)   #输入的参数   和   特征的个数（维度）
    ...
    def initial_label_enumeration(self):
        ...
        self.global_labels = set()  #self.global_labels记录的是特征的维度
        for graph_pair in tqdm(graph_pairs):
            ...
            self.global_labels = self.global_labels.union(set(data["labels_1"]))#用set来去重（在一个集合中不会允许出现同的元素）
            self.global_labels = self.global_labels.union(set(data["labels_2"]))
        self.global_labels = sorted(self.global_labels)
        #使用enumerate方法给不重复的属性增加索引（进行排序）
        self.global_labels = {val:index  for index, val in enumerate(self.global_labels)}#self.global_labels记录的是特征的维度
        #这里的label其实不是标签，就是节点的属性，这里      获取节点属性有多少种类型属性          然后传入到143行setup_model（）作为参数
        self.number_of_labels = len(self.global_labels)#    即维度的大小

    #如何划分batch；分批划分数据（输出为python列表格式）
    def create_batches(self):
        ...
        batches = []
        for graph in range(0, len(self.training_graphs), self.args.batch_size): #按照batch-size划分batch
    ...
    #将数据转化为pyg需要的那种格式（转置时核心思想）          GCN数据处理之前要求的格式      是Mytest.txt的
    def transfer_to_torch(self, data):
        ...
        """
        new_data = dict()
        #该项目场景为有向图  Mytest.txt【0，8】变为【0，8】【8，0】
        ...
        norm_ged = data["ged"]/(0.5*(len(data["labels_1"])+len(data["labels_2"])))  #取两个标签长度的平均值-原文提到
    ...
    def process_batch(self, batch):
        ...
        losses = 0
        #??????????????????????
        for graph_pair in batch:    #对这个batch进行遍历
            ...
            #   损失函数时MSE
            losses = losses + torch.nn.functional.mse_loss(data["target"], prediction) #计算MSE
    ...
    def fit(self):
        ...
        self.optimizer = torch.optim.Adam(self.model.parameters(),  #第一个参数：模型的参数
                                          lr=self.args.learning_rate,   #第二个参数：学习率
                                          weight_decay=self.args.weight_decay)  #第三个参数：衰减率
        # -------------------pythrch固定语法-----
        self.model.train()  #进入到训练模式
        epochs = trange(self.args.epochs, leave=True, desc="Epoch")#tqdm库   显示一个进度条
        #-----------------------pythrch固定语法---------------------------
        for epoch in epochs:    #默认轮数是5
            batches = self.create_batches() #·进行分批（分为不同的batch）计算？？？？？？？？？
            self.loss_sum = 0
            main_index = 0
            #到此 已经打包
            #然后对每个包（batch）进行操作
            for index, batch in tqdm(enumerate(batches), total=len(batches), desc="Batches"):
                ...
                self.loss_sum = self.loss_sum + loss_score * len(batch)#    将之前计算出的MSE与Batch相乘再加上loss
                #   MSE的实现
                loss = self.loss_sum/main_index #  将loss_sum除以中的数量得到loss
    ...
    def score(self):
        ...
        for graph_pair in tqdm(self.testing_graphs):
            ...
            self.scores.append(calculate_loss(prediction, target))# 将prediction和target进行比较得到loss
