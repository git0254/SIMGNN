...
def parameter_parser():#封装有所有命令行参数定义的方法
    ...
    parser.add_argument("--filters-1",
                        type=int,
                        default=128,
	                help="Filters (neurons) in 1st convolution. Default is 128.")# 第一层卷积网的特征数量（默认128条）
    ...
    parser.add_argument("--learning-rate",
                        type=float,
                        default=0.001,
	                help="Learning rate. Default is 0.001.")# 学习速度

    parser.add_argument("--weight-decay",
                        type=float,
                        default=5*10**-4,
	                help="Adam weight decay. Default is 5*10^-4.")# 防止死记硬背的"防沉迷系数"（衰减率）
