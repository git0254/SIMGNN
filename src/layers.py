...
class AttentionModule(torch.nn.Module):
    ...
    def setup_weights(self):
        ...
        Defining weights.
        """
        #   ×矩阵进行变换     filters_3 x filters_3的矩阵
    ...
    def forward(self, embedding):   #embedding即左右节点的输入
        ...
        #global_context 即c
        # matmul也是矩阵相乘      weight_matrix即W2
        global_context = torch.mean(torch.matmul(embedding, self.weight_matrix), dim=0) #embedding矩阵和matrix矩阵进行计算
        ...
        #得到最终的Graph Embedding: Global Context-Aware Attention       最后一步x每个节点的embedding
...
class TenorNetworkModule(torch.nn.Module):
    ...
    #？？？？？？？？？？？？？？？？
    def forward(self, embedding_1, embedding_2):    #前面得到的embedding_1尺度为f3*1要变成1*f3
        """
        ...
        #   先用T转置       -1表示f3*Tensor network的个数
        # 计算embedding_1和weight_matrix的乘积
